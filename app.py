...
# 南京科技职业学院 固定默认坐标（不记忆、不保存）
NJKY_DEFAULT_A = (32.2325, 118.7488)
NJKY_DEFAULT_B = (32.2348, 118.7498)
NJKY_CENTER = (32.2335, 118.7492)

# ===================== 初始化状态（每次打开强制重置，不读文件、不记忆） =====================
if "init_done" not in st.session_state:
    st.session_state.A = NJKY_DEFAULT_A
    st.session_state.B = NJKY_DEFAULT_B
    ...
    st.session_state.obs_h = 20
    st.session_state.last_click_time = 0
    st.session_state.safe_radius = 0.0002
    ...
    st.session_state.flight_waypoints = []
...
if page=="航线规划":
    ...
    with col_ctrl:
        ...
        if st.button("🗑️ 清空全部障碍物"):
            ...
            st.session_state.temp_points=[]
            st.rerun()
        st.info(f"当前临时障碍物总数：{len(st.session_state.polygon_memory)} 个")
        st.divider()
        ...
        if st.session_state.A_set and st.session_state.B_set:
            _, conflict = get_safe_route(st.session_state.A, st.session_state.B, st.session_state.polygon_memory, st.session_state.safe_radius, st.session_state.route_side)
            if conflict:
                st.warning(f"🛡️ 航线与障碍物距离不足，已按「{st.session_state.route_side}」方向生成安全绕飞路线")
                need_avoid = True
            else:
    ...
    with col_map:
        ...
        if st.session_state.A_set:
            folium.Marker(A_wgs, icon=folium.Icon(color='red', icon='plane', prefix='fa'), popup="飞行起点A").add_to(m)
        if st.session_state.B_set:
            folium.Marker(B_wgs, icon=folium.Icon(color='green', icon='plane', prefix='fa'), popup="飞行终点B").add_to(m)
        for idx,obs in enumerate(st.session_state.polygon_memory):
            pts=obs["pts"]
            hh=obs["h"]
            if len(pts)>=3:
